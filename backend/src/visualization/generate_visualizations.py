...
def items_most_rated_sns(ratings):
    value_counts_items = (
        ...
        .value_counts()[0:10]
        .reset_index()
        .rename(columns={"index": "item", "item": "count"})
    )
...
def items_most_rated(ratings):
    ...
    path_to_save = (
        ...
        .joinpath(visualization_output_path)
...
rmse_values = np.array([1.458033, 1.375486, 1.344586, 1.360211])

ndcg_values = np.array([0.058862, 0.062897, 7.274982, 6.960768])


mae_values = np.array([1.161295, 1.109742, 1.095435, 1.102076])
...
indexes = ["ItemKNN", "UserKNN", "Bias", "BiasedSVD"]
...
def evaluate_measures_results(measure_df, metric):
    ...
    ax.set_title(
        "Comparativo entre valores de {} para os algoritmos de recomendação".format(
            metric
        )
    )
