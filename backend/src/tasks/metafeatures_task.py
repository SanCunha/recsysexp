...
class MetaFeaturesTask(Task):
    def __init__(self, metafeatures=None, args=None):
        ...
        self.metrics_calculator_path = hrf_external_path().joinpath(
            "metric_calculator/"
        )
        ...
        self.metrics_calculator_config_file_path = (
            self.metrics_calculator_path.joinpath("metric_calculator_example.xml")
        )

    def create_command_to_metrics_calculator(self):
        command = "java -jar {} {}".format(
            self.metrics_calculator_jar_path, self.metrics_calculator_config_file_path
        )
        return command
