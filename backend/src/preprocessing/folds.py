...
class Strategy(ABC):
    @abstractmethod
    def create_folds(
        self, X, y, n_splits: int, shuffle: bool, random_state: int, **kwargs
    ):
        pass
...
class FoldsProcessing(AbstractPreProcessing):
    def __init__(self, parameters: dict) -> None:
        ...
        self.train_folds_output_directory = (
            hrf_experiment_output_path()
            .joinpath(self.folds_output_directory)
            .joinpath("train/")
        )

        self.validation_folds_output_directory = (
            hrf_experiment_output_path()
            .joinpath(self.folds_output_directory)
            .joinpath("validation/")
        )
    ...
    def pre_processing(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        vfo_dir_exists = check_if_directory_exists(
            self.validation_folds_output_directory
        )
        ...
        for train_index, validation_index in folds_indexes:
            ...
            validation_folds_archive_name = (
                self.validation_folds_output_directory.joinpath(
...
class Folds:
    ...
    def create_folds(
        ...
        @return:
        """
        result = self._strategy.create_folds(
            self,
            X=X,
            y=y,
            n_splits=n_splits,
...
class KFoldStrategy(Strategy):
    def create_folds(
        self, X, y, n_splits: int, shuffle: bool, random_state: int = None, **kwargs
    ):
...
class GroupKFoldStrategy(Strategy):
    def create_folds(
        ...
        return group_kfold.split(X=X, y=y)
...
class StratifiedGroupKFoldsStrategy(Strategy):
    def create_folds(
        ...
        stratified_group_kfolds = StratifiedGroupKFold(
            n_splits=n_splits, shuffle=shuffle, random_state=random_state
        )
        return stratified_group_kfolds.split(X=X, y=y)
...
class StratifiedShuffleSplitStrategy(Strategy):
    def create_folds(
        ...
        return stratified_shuffle_split.split(X=X, y=y)
...
class ShuffleSplitStrategy(Strategy):
    def create_folds(
        ...
        return shuffle_split.split(X=X, y=y)
...
class StratifiedKFoldStrategy(Strategy):
    def create_folds(
        ...
        return stratified_kfold.split(X=X, y=y)
